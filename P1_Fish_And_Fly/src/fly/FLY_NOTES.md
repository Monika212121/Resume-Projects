...
AsyncOrchestrator
   ↓
Tasks (Fish / Fly / Manatee)
   ↓
Queues + Mission Control
```
...
Fish:
  collects → reaches 10kg
  ↓
  unloads → nearest dump
  ↓
  sends heartbeat:
      dump_event { dump_id, load }
...
AVAILABLE → (dispatch) → LOCKED → (manatee done) → AVAILABLE